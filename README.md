...
本システムは、J-Quantsの有料枠を使わずに、以下の無料公開データソースを優先してバックテストを実行できるように設計されています。

1. **JPX Listed Issues (東証上場銘柄一覧)**: 東証が公式サイトで提供する銘柄一覧Excelファイルから、銘柄マスター (`Symbol`, `SymbolMasterHistory`) を自動構築します。
2. **Yahoo Finance**: npmパッケージ `yahoo-finance2` および Python `yfinance` フォールバックを用いて、日本株（`.T` ティッカー）の日足、配当、分割、および簡易的な財務情報、決算発表予定日を取得します。
3. **EDINET**: `EDINET_API_KEY` を設定することで、有価証券報告書等の書類の提出メタデータを `DisclosureDocument` に取り込み、AIが特徴量として利用できる形にします。

#### ⚠️ Yahoo Financeデータの利用に関する免責事項
Yahoo Financeから取得されるデータは非公式な方法を用いており、利用条件や仕様変更、リクエスト制限（Rate Limit）の影響を受けやすい特徴があります。
- **個人研究・検証用途**としてのみご使用ください。
- **商用利用や高頻度のスクレイピング**は絶対に避けてください。
- データの欠損や株価分割調整の不整合が起きる可能性があるため、バックテストの実行前には必ず「データ品質」画面で整合性の確認（DataQualityService）を行うことを推奨します。

### 無料データの初期セットアップ (Bootstrap)

データベースを初期化したあと、または新しい銘柄を無料データから取得する場合は、以下のコマンドまたはブラウザのUIから一括取得（ブートストラップ）を行うことができます。

```bash
# 無料データの一括初期構築ジョブを実行 (APIサーバー起動中)
pnpm data:bootstrap:free
```
または、Web UIのサイドバーメニュー「**無料一括データ構築**」(`/data/bootstrap`) から市場区分、期間、最大銘柄数を指定して実行できます。
 J-Quants/TDnetは将来の有料高品質Providerとして残されています。
